...
//     keys = try? CC.EC.generateKeyPair(384)
...
//        self?.getSecretKeyFromSWCrypt(result: result,argument:call.arguments as! String)
        ...
        self!.secretKeyString = (args["secretKey"] as! String)
        self?.getcardDetailString(result: result,argument:detailMessage)
//        self?.getcardDetailStringSWCrypto(result: result,argument:detailMessage)
 ...
 private func getSecretKeyString(result: FlutterResult,argument:String) {
    ...
    private func getSecretKeyFromSWCrypt(result: FlutterResult,argument:String){
        let shared = try? CC.EC.computeSharedSecret(keys!.0, publicKey:  Data(stringToBytes(argument )!))
        print(shared)
        result(shared)
    }
...
private func getcardDetailString(result: FlutterResult,argument:String) {
    ...
    private func getcardDetailStringSWCrypto(result: FlutterResult,argument:String) {
       
        let decryptedResult = argument.aes_Decrypt(AES_KEY:self.secretKeyString.bytes)
